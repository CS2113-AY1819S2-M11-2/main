...
****
To force Gradle to create the JAR file even if the current one is up-to-taskDate, you can '`clean`' first. +
...
Runs all tests.
* **`guiTests`** +
Runs all tests in the `seedu.priority.ui` and `systemtests` package
* **`nonGuiTests`** +
Runs all non-GUI tests in the `seedu.priority`
package
* **`headless`** +
